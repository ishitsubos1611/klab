    ...
    $("#calender td").on("click",function(){
      ...
      for(var i in data){
        if (cellnum == previousNum){
          ...
          previousNum = "";
          break;
        }else if(cellnum == data[i].date　|| data[i].date == "9月2日"){
          $('#cal').append('<div id="guide" class="info"><p>　(人気)　'+data[i].guidelocation+'ガイド </p>'
          +'<table id="states"><tr><td>Date</td><td>'+monthVal+'月'+dayVal+'日</td></tr><tr><td>GuidScheduleID</td><td>'+data[i].GuidScheduleID+'</td></tr><tr><td>Boarding time</td><td>'+data[i].start_time+'</td></tr>'
